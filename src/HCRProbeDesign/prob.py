...
def make_gaussian(mu, sigma):
    ...
    """
    return lambda x, mu=mu, sigma=sigma: (
...
def variance(l, failfast=1):
    if (not l) or len(l) == 1:
        if failfast:
            raise "tools.variance: Not enough samples.  Need >= 2, got %s" % len(l)
        else:
            return 0  #'N/A'
    m = avg(l, 1)
    s = 0
...
def binomial_likelihood_ratio(ps, k, n):
    ...
    # return p
    if likelihoods[0]:
        return np.log(likelihoods[1]) / likelihoods[0]
    else:
