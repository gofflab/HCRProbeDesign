...
from . import HCR, sequencelib, thermo, utils
...
class Tile:
    def __init__(self, sequence, seqName, startPos):
        ...
        self.name = (
            f"{self.seqName}:{self.start}-{self.start+len(self.sequence)}".replace(
                " ", "_"
            )
        )
        self.masked = False
        self.hitCount = (
            -1
        )  # -1 indicates that genome masking has not yet been performed.
    ...
    def overlaps(self, b):
        ...
        if (self.start <= b.start and b.start <= self.end) or (
            self.start >= b.start and self.start <= b.end
        ):
            return True
        else:
            return False
    ...
    def __cmp__(self, other):
        return cmp(
            (self.seqName, self.startPos, self.name),
            (other.seqName, other.startPos, other.name),
        )
    ...
    def calcGibbs(self):
        ...
        binding_energy = thermo.salt_adjust(
            binding_energy / 1000, len(self.sequence), saltconc=0.33
        )  # kcal/mol
    ...
    def calcdTm(self):
        self.dTm = abs(
            primer3.calcTm(self.fivePrimeSeq) - primer3.calcTm(self.threePrimeSeq)
        )
